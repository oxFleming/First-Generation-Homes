...
export const ProjectDetails: React.FC<ProjectDetailsProps> = ({ project, onClose }) => {
  ...
  return (
    <motion.div 
      ...
      className="fixed inset-0 z-[100] bg-[#f4f3ef] overflow-y-auto overflow-x-hidden text-[#141414]"
    >
      <div className="min-h-screen w-full flex flex-col md:flex-row">
        {/* Close Button */}
        <button 
          onClick={onClose}
          className="fixed top-8 right-8 z-50 w-12 h-12 rounded-full bg-black/5 backdrop-blur-md flex items-center justify-center hover:bg-black/10 transition-colors"
        >
          <X size={24} />
        </button>
        ...
        <div className="w-full md:w-[40%] lg:w-[35%] p-8 md:p-16 relative">
          <div className="md:sticky md:top-32 flex flex-col gap-8">
            ...
            <div className="flex gap-4 text-xs font-mono tracking-widest uppercase opacity-60">
              <span>[ {project.location} ]</span>
              <span>[ 2024 ]</span>
            </div>

            <p className="text-base md:text-lg leading-relaxed max-w-md mt-8 font-light">
